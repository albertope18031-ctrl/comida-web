...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <>
      <div className="group relative flex flex-col overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm transition-all duration-300 hover:shadow-xl hover:border-emerald-700/40">
        ...
        >
          ...
          <div className="absolute top-3 left-3 flex flex-wrap gap-1.5 z-10">
            {product.popular && (
              <Badge variant="gold" className="shadow-md flex items-center gap-1">
                <Sparkles className="h-3 w-3" />
                Popular
              </Badge>
            )}
            {product.isCombo && (
              <Badge className="bg-[#005A36] text-white shadow-md">
                Combo Completo
              </Badge>
            )}
          </div>
        ...
        <div className="flex flex-1 flex-col p-5">
          ...
          <div className="mt-4 flex items-center justify-between pt-4 border-t border-neutral-100">
            ...
            <Button
              ...
              className="font-bold flex items-center gap-1.5 shadow-sm rounded-lg"
            >
      ...
      {/* Dynamic Product Customizer Modal */}
